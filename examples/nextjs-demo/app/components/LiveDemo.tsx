  ...
  return (
    <section id="demo" className="mx-auto max-w-7xl px-6 py-32 lg:px-8">
      <div className="flex flex-col items-center">
        ...
        <h2 className="mt-4 text-4xl font-extrabold tracking-tight sm:text-5xl">Experience our integration kit.</h2>
        <p className="mt-4 text-text-secondary">Fully functional, production-grade zero-knowledge signing demo using live wallet adapters.</p>
      </div>
              ...
              <div className="grid grid-cols-2 gap-3 pt-4">
                {!isConnected ? (
                  adapters.map(adapter => (
                    <button 
                      key={adapter.name}
                      onClick={() => realConnect(adapter.name)}
                    ...
                    >
                      {connectLoading ? "Connecting..." : `Connect ${adapter.name}`}
                    </button>
                  ))
                ) : (
                  <button 
